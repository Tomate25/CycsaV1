...
<style>
    ...
    .dia-celda { background: #ffffff; min-height: 95px; padding: 6px; display: flex; flex-direction: column; position: relative; transition: background 0.2s; box-sizing: border-box; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCalendar() {
        ...
        for (let day = 1; day <= totalDays; day++) {
            ...
            let eventsHtml = '<div class="eventos-lista">';
            dayEvents.forEach(ev => {
                ...
                const label = `${ev.codigo_muestra} (${ev.identificador_especimen}, ${ev.edad_dias}d)`;
                eventsHtml += `<a href="/Cycsa/publico/laboratorio/detalle-muestra?id_lote=${ev.id_lote}" class="evento-badge ${badgeClass}" title="${ev.nombre_ensayo} - Muestra Campo: ${ev.codigo_campo}">${label}</a>`;
            });
            eventsHtml += '</div>';
