...
<form action="/Cycsa/publico/productos/crear" method="POST">
    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

    <div class="seccion-form">
        <h3 class="seccion-titulo"><i class="fa-solid fa-circle-info"></i> Información General del Ensayo / Servicio</h3>
        ...
        <div class="grid-3">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nombre_comercial" class="form-control" required placeholder="Ej: Humedad en Agregados - ASTM C566-25" value="<?= htmlspecialchars($valores['nombre_comercial'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            </div>

            <div class="form-group">
                <label class="form-label">Código de Servicio / Formato</label>
                <input type="text" name="codigo_servicio" class="form-control" placeholder="Ej: CYCSA-RT-FM-22G" value="<?= htmlspecialchars($valores['codigo_servicio'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            </div>
        </div>

        <div class="form-group">
            <label class="form-label">Descripción Técnica o Servicio a Realizar *</label>
            <textarea name="ensayo_servicio" class="form-control" required rows="2" placeholder="Ej: CYCSA-PE-01-Determinación de Humedad en muestras de Agregados."><?= htmlspecialchars($valores['ensayo_servicio'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>

        <div class="grid-3">
            <div class="form-group">
                <label class="form-label">Matriz / Grupo</label>
                <input type="text" name="matriz_tipo" id="matriz_tipo" class="form-control" placeholder="Ej: Agregados, Suelo, Concreto" list="categorias_existentes" value="<?= htmlspecialchars($valores['matriz_tipo'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            ...
            <div class="form-group">
                ...
                <input type="text" name="tipo_muestra" class="form-control" placeholder="Ej: Agregados Finos" value="<?= htmlspecialchars($valores['tipo_muestra'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            </div>

            <div class="form-group">
                <label class="form-label">Tipo de Muestreo</label>
                <input type="text" name="tipo_muestreo" class="form-control" placeholder="Ej: Aleatorio / Puntual" value="<?= htmlspecialchars($valores['tipo_muestreo'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            </div>
        </div>
    </div>
    ...
    <div class="seccion-form">
        ...
        <div class="grid-3">
            <div class="form-group">
                ...
                <select name="estatus" class="form-control">
                    <option value="No acreditado" <?= (isset($valores['estatus']) && $valores['estatus'] === 'No acreditado') ? 'selected' : '' ?>>No Acreditado</option>
                    <option value="Acreditado" <?= (isset($valores['estatus']) && $valores['estatus'] === 'Acreditado') ? 'selected' : '' ?>>Acreditado (ISO 17025)</option>
                </select>
            </div>

            <div class="form-group">
                <label class="form-label">Norma ASTM</label>
                <input type="text" name="norma_astm" class="form-control" placeholder="Ej: ASTM C566-25" value="<?= htmlspecialchars($valores['norma_astm'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            </div>

            <div class="form-group">
                <label class="form-label">Procedimiento Muestreo (CYCSA-PE)</label>
                <input type="text" name="procedimiento_muestreo" class="form-control" placeholder="Ej: CYCSA-PE-01" value="<?= htmlspecialchars($valores['procedimiento_muestreo'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            </div>
        </div>
        ...
        <div class="grid-3">
            <div class="form-group">
                ...
                <input type="text" name="codigo_hoja_campo" class="form-control" placeholder="Ej: CYCSA-RT-FM-13" value="<?= htmlspecialchars($valores['codigo_hoja_campo'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
            </div>

            <div class="form-group">
                <label class="form-label">Unidad de Medida *</label>
                <input type="text" name="unidad_medida" class="form-control" required placeholder="Ej: Unidad, Viaje, Metro" value="<?= htmlspecialchars($valores['unidad_medida'] ?? 'Unidad', ENT_QUOTES, 'UTF-8') ?>">
            </div>
        ...
        <div class="grid-2">
            <div class="form-group">
                ...
                <textarea name="condiciones_muestra" class="form-control" rows="3" placeholder="Ej: Tienen que venir o ser tomadas en bolsas o recipientes que mantengan la humedad..."><?= htmlspecialchars($valores['condiciones_muestra'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>

            <div class="form-group">
                <label class="form-label">Tiempo de Entrega / Observaciones</label>
                <textarea name="observaciones" class="form-control" rows="3" placeholder="Ej: (2 días hábiles) o A convenir"><?= htmlspecialchars($valores['observaciones'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>
        </div>
    </div>
